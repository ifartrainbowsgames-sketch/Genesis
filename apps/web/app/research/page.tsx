  ...
  return (
    <main className="shell">
      ...
      <div className="researchGrid">
        <section className="panel researchControls">
          ...
          <form className="researchForm" onSubmit={runResearch}>
            ...
            <p className="mutedCopy">Genesis asks only the configured SearXNG broker for search results. The model receives result titles, URLs and snippets; it does not silently browse arbitrary pages.</p>
          </form>
        </section>

        <section className="panel researchAnswer">
          ...
          {!result && <div className="researchEmpty">The source-grounded synthesis will appear here with inline source IDs such as [S1].</div>}
          {result && (
            <div className="researchAnswerBody">
              <div className="researchMeta">task {result.task_id}</div>
              <h2>{result.report.query}</h2>
              <p className="researchText">{result.report.answer}</p>
      ...
      <section className="panel sourcesPanel">
        ...
        <div className="sourceGrid">
          {!result && <div className="researchEmpty">Search-result sources will be preserved here so you can inspect exactly what the Researcher used.</div>}
          {result?.report.sources.map((source) => (
            <article className="sourceCard" key={source.id}>
              <div className="sourceHead"><b>[{source.id}]</b><span>{source.engine || "search"}</span></div>
